...
class MissingelectiondocumentsController extends Controller
{
    ...
    {
        $query = DB::table('missing_election_details')->get();
    ...
    return view('admin::missingelectiondocuments.manualdata', compact('category_options', 'year_options','hut_id'));
    ...
    public function storeManualData(Request $request)
    {
        ...
        DB::table('missing_election_details')
                ->where('hut_id', $hutId)
        ...
        return redirect()->route('admin.missingelectiondocuments.index')->with('status', 'Document updated Successfully!');
