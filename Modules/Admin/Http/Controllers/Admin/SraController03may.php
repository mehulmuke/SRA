...
*/
    ...
    public function uplodemissingdocument($hut_id)
    {
        ...
        return view('admin::sra.uplodemissingdocument', compact('category_options', 'year_options','hut_id'));
    }
        ...
        if(isset($categories)){
            foreach($categories as $data)
            {
                ...
                if ($data == 'Other') {
                    ...
                    $other_document = $request->file('other_document_file');
                    $new_other_document = '';
        ...
        $remark = $request->input('remark');
         ...
         switch ($request->input('action')) {
            ...
            DB::table('sims_master')
                ->where('hut_id', $hid)
                 //->where('user_id',auth()->user()->id)
                ->update(['overall_status' => $status,'remark'=>$remark,'updated_at'=>NOW(),'status'=>'1']);
