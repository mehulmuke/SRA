...
if ($('.file-folder-tree').length !== 0) {
    ...
    let breadcrumbsClass = '.fftub';

    new FileFolderTree(tree,breadcrumbsClass);
}
if ($('.file-folder-tree-table').length !== 0) {
    let tree = $('.file-folder-tree-table');
    let breadcrumbsClass = '.fft-table';
    new FileFolderTree(tree,breadcrumbsClass);
}
